...
exports.addUser = function(req, res) {
    var user = req.body;
    var encPassword = SHA1(req.body.uPassword);
    var encUser = {
    ...
    db.collection('users', function(err, collection) {
        collection.insert(encUser, {safe:true}, function(err, result) {
            if (err) {
                res.send({'Status':'Error','Message':err});
            } else {
                res.send({'Status':'Success',"Message":"User has been added successfully"});
            }
        });
    });
...
exports.login = function(req, res)  {
    ...
    db.collection('users', function(err, collection) {
        collection.findOne({'uEmail': userId, 'uPassword':encHashCode}, function(err, userProfile)  {
            if(err) {
                console.log("ERROR");
            }
            if(!userProfile)   {
                res.send({Status:"Error", Message: "Incorrect ID and Password"});
            }
            else    {
